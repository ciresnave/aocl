...
//!
//! Provides a [`Scalar`] trait implemented for `f32` and `f64` so that
//! GEMM, AXPY, and DOT can be called with either precision. Wrappers
//! take Rust slices (with bounds-checked dimension arguments) instead of
//! raw pointers; mismatched sizes return an [`Error::InvalidArgument`].
//!
//! For routines not yet wrapped here, drop down to [`aocl-blas-sys`]
//! for direct CBLAS access.
...
/// Scalar element type usable with the wrapped BLAS routines.
///
/// Implemented for `f32` and `f64`; complex precisions will follow.
...
/// Convenience entry point: `C := α · op(A) · op(B) + β · C` for
/// tightly-packed row-major matrices.
#[allow(clippy::too_many_arguments)]
