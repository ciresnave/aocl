...
//!
//! Provides one-shot, allocate-and-return helpers around AOCL's unified
//! `aocl_llc_*` API. Each call manages its own `aocl_compression_desc`
//! handle and `aocl_llc_setup` / `aocl_llc_destroy` lifecycle internally.
//!
//! For streaming use, custom work-buffers, or codec-specific tuning, drop
//! down to [`aocl_sys::compression`].

use crate::error::{Error, Result};
use aocl_sys::compression as sys;
...
fn empty_desc() -> sys::aocl_compression_desc {
    ...
    // It contains pointers (which become null) and primitive numerics
    // (which become zero); AOCL's setup/compress/decompress routines
    // accept null work/in/out pointers and only deref what they need
    // based on the operation requested.
    unsafe { std::mem::zeroed() }
}

/// Compute an upper bound on the compressed size for `src_len` bytes
/// under the given codec. Useful for sizing destination buffers.
pub fn compress_bound(codec: Codec, src_len: usize) -> Result<usize> {
    // SAFETY: free function with no preconditions; takes scalar args only.
...
///
/// `level` is the codec-specific compression level; pass `0` for default.
/// `optLevel` is set to `2` (AVX) which AOCL accepts on every modern AMD
/// CPU; AOCL itself decides the best implementation at runtime.
pub fn compress(codec: Codec, src: &[u8], level: u32) -> Result<Vec<u8>> {
    ...
    // SAFETY: the descriptor's `inBuf` / `outBuf` aliases the borrowed
    // slices for the duration of these calls only; we don't store the
    // descriptor or pointers anywhere they could outlive the slices.
...
///
/// AOCL's unified decompress API requires the caller to know the
/// destination size up front (most codecs above don't store it
/// implicitly). Pass `dst_len` = the expected uncompressed length, or
/// the maximum size you're willing to accept.
...
pub fn version() -> Option<String> {
    // SAFETY: returns a pointer to a NUL-terminated static string owned
    // by the library; we never free it.
    unsafe {
