...
//!
//! Provides the [`Scalar`] trait implemented for `f32` and `f64`, and a few
//! convenience entry points for the most common factorizations and solves.
//!
//! For routines not yet wrapped here, drop down to [`aocl_sys::libflame`].

use crate::error::{Error, Result};
use aocl_sys::libflame as sys;

const ROW_MAJOR: i32 = 101;
const COL_MAJOR: i32 = 102;

/// Storage order of a matrix passed to LAPACK.
...
/// Scalar element type usable with the LAPACK routines exposed here.
pub trait Scalar: Copy + Sized + private::Sealed {
    /// Solve `A · X = B` for X, overwriting `B` with the solution and `A`
    /// with its LU factorization. `ipiv` receives the pivot indices.
    ...
    /// Compute the LU factorization of an `m × n` matrix `A` with partial
    /// pivoting. On exit, `A` contains `L` and `U` (`L` has unit diagonal,
    /// not stored); `ipiv` contains the row permutation.
    fn getrf(
        layout: Layout,
        m: usize,
        n: usize,
    ...
    ($t:ty, $gesv:ident, $getrf:ident) => {
        impl Scalar for $t {
            fn gesv(
                ...
                // SAFETY: pointers come from valid mutable slices of the
                // bounds-checked sizes.
                let info = unsafe {
                    sys::$gesv(
                        layout.raw(),
                        n as i32,
...
///
/// On success, `b` is overwritten with the solution `X`, and `a` with its
/// LU factorization.
pub fn gesv<T: Scalar>(n: usize, a: &mut [T], ipiv: &mut [i32], b: &mut [T]) -> Result<()> {
    // For a single right-hand side stored as an n-vector, nrhs = b.len() / n.
    if n == 0 {
        return Ok(());
    }
    let nrhs = b.len() / n;
...
mod tests {
    ...
    fn getrf_then_back_solve_via_dim_check() {
        ...
        // Just verify call-through; numerical correctness of factorization
        // is exercised indirectly via gesv tests above.
        assert!(ipiv.iter().all(|&p| p > 0));
    }

    #[test]
    fn dim_mismatch_is_error() {
        let mut a = [1.0_f64; 3]; // too small for 2x2
        let mut ipiv = [0_i32; 2];
        let mut b = [1.0_f64, 2.0];
        let err = gesv(2, &mut a, &mut ipiv, &mut b).unwrap_err();
