...
pub struct Digest {
    ...
    /// Backing storage for the ALCP context. Lives at least as long as
    /// `handle.context` is non-null. We use a `Box<[u8]>` rather than a
    /// `Vec<u8>` to make the buffer's address stable for the lifetime of
    /// this struct.
    _context: Box<[u8]>,
    output_len: usize,
    finished: bool,
...
impl Digest {
    ...
    pub fn new(mode: Mode) -> Result<Self> {
        // SAFETY: zero-arg query of a static size.
        ...
        // SAFETY: handle.context points into our owned `context` buffer
        // which we keep alive for the lifetime of this struct.
    ...
    /// Feed a chunk of data into the digest. May be called multiple times.
    pub fn update(&mut self, data: &[u8]) -> Result<()> {
        ...
        // SAFETY: handle remains valid; data ptr+len describe a live slice.
        check_error(unsafe {
            sys::alcp_digest_update(&mut self.handle, data.as_ptr(), data.len() as u64)
        })
    }

    /// Finalize and return the digest. Consumes `self` so the same
    /// instance cannot be reused.
    pub fn finalize(mut self) -> Result<Vec<u8>> {
        let mut out = vec![0u8; self.output_len];
        // SAFETY: out has exactly output_len bytes; handle is still valid.
        let res = unsafe {
...
impl Drop for Digest {
    fn drop(&mut self) {
        // SAFETY: alcp_digest_finish releases internal allocations linked
        // to handle.context but does not free our `_context` buffer.
        // Idempotent across multiple calls per AMD docs.
