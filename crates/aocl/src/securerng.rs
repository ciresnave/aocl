...
//!
//! Wraps the AMD secure-RNG entry points around the x86 `RDRAND` (already
//! conditioned, suitable for general-purpose random) and `RDSEED` (raw
//! entropy, suitable for seeding cryptographic generators) instructions.
//!
//! All routines may fail on hardware that lacks these instructions, so
//! every API returns a [`Result`] and uses [`Error::Status`] to convey
//! `SECRNG_NOT_SUPPORTED` / `SECRNG_FAILURE`. Use [`is_rdrand_supported`]
//! / [`is_rdseed_supported`] to probe ahead of time.

use crate::error::{Error, Result};
use aocl_sys::securerng as sys;
...
/// `RDSEED`. Per Intel's guidance ten retries cover essentially all
/// transient cases.
...
pub fn is_rdrand_supported() -> bool {
    // SAFETY: pure CPUID query, no preconditions.
...
/// Read a single random `u16` from `RDRAND`, retrying up to `retry_count`
...
/// Read a single random `u16` from `RDSEED` (raw entropy source).
...
pub fn version() -> Option<String> {
    // SAFETY: returns a pointer to a NUL-terminated static string owned by
    // the AOCL-SecureRNG library; we never free it.
    unsafe {
...
mod tests {
    ...
    fn cpu_support_query_is_consistent() {
        // Just verify the calls succeed and return a stable value.
    ...
    fn rdrand_yields_distinct_values_when_supported() {
        ...
        // Probability of collision is 2^-64; if you fail this test you
        // either won the lottery or RDRAND is broken.
    ...
    fn rdrand_bytes_fills_buffer() {
        ...
        // Almost certainly not all zeros after a fill.
