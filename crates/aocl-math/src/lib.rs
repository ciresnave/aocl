//! Safe Rust wrappers for AOCL-LibM — vectorized scalar math, AMD-tuned.
//!
//! Each function maps to a single `amd_*` entry point from `amdlibm.h`.
//! The `f32` variants forward to the corresponding `amd_*f` symbol.
//!
//! This crate currently exposes only the scalar API. The vector API in
//! `amdlibm_vec.h` is gated by AMD as experimental; it will be added
//! behind a sub-feature once stabilized.
