//! Cross-component error type shared by every `aocl-*` safe wrapper.
...
///
/// Each AOCL component has its own native error code conventions; this
/// enum is the lowest common denominator. Component-specific modules layer
/// their own typed errors on top where useful.
#[derive(Debug, Error)]
#[non_exhaustive]
pub enum Error {
    ...
    Status {
        /// AOCL component that produced the error (e.g. `"blas"`, `"lapack"`).
    ...
    /// Invalid arguments passed by the caller (dimension mismatch, null
    /// buffer, etc.). These are detected client-side before invoking AOCL.
    ...
    /// A C string returned by AOCL was missing its NUL terminator within
